...
``aad_client`` is a volunteer maintained open source project and we welcome contributions of all forms. The sections
below will help you get started with development, testing, and documentation. We’re pleased that you are interested in
working on ``aad_client``. This document is meant to get you setup to work on ``aad_client`` and to act as a guide and reference
to the development setup. If you face any issues during this process, please
`open an issue <https://github.com/djpugh/aad_client/issues/new?title=Trouble+with+development+environment>`_ about it on
the issue tracker.

Setup
~~~~~

``aad_client`` is an extension component to :pypi:`fastapi` to provide Azure Active Directory authentication,
written in Python. To work on it, you'll need:

- **Source code**: available on `GitHub <https://github.com/djpugh/aad_client>`_. You can use ``git`` to clone the
    repository:

  .. code-block:: console

      git clone https://github.com/djpugh/aad_client
      cd aad_client
...
``aad_client`` tests are written using the :pypi:`pytest` test framework. :pypi:`tox` is used to automate the setup
and execution of ``aad_client`` tests.
...
``aad_client`` is very tightly coupled to Azure Active Directory, so most of the full tests follow manual testing.
This needs configuring with Azure Active Directory App Registrations (see :ref:`config-aad-appreg`) and


Running linters
~~~~~~~~~~~~~~~

``aad_client`` uses :pypi:`flake8` and extensions for managing linting of the codebase. ``flake8`` performs various checks on all
files in ``aad_client`` and uses tools that help follow a consistent code style within the codebase. To use linters locally,
run:
...
``aad_client`` documentation is built using :pypi:`Sphinx`. The documentation is written in reStructuredText. To build it
locally, run:
...
have legal permission to distribute any code you contribute to ``aad_client`` and it must be available under the MIT
License. Provide tests that cover your changes and run the tests locally first. ``aad_client``
